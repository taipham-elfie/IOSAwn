...
public class SQLitePrimitivesDB {
    ...
    private func getAllValuesStarting<T>(
        with keyFragment:String,
        tableName: String,
        tag: String,
        valueGetter: (OpaquePointer) -> T
    ) -> Result<[String: T], SQLiteError> {
        
        var values: [String: T] = [:]
        let query = """
            SELECT \(COLUMN_KEY), \(COLUMN_VALUE) FROM \(tableName)
            WHERE \(COLUMN_TAG) = '\(tag)'
            AND \(COLUMN_KEY) like '\(keyFragment)%'
        """
    ...
    public func getAllStringValuesStarting(with keyFragment:String, tag: String) -> Result<[String: String], SQLiteError> {
        return getAllValuesStarting(with: keyFragment, tableName: TABLE_STRING, tag: tag) { statement in
            String(cString: sqlite3_column_text(statement, 1))
        }
    }

    public func getAllFloatValuesStarting(with keyFragment:String, tag: String) -> Result<[String: Float], SQLiteError> {
        return getAllValuesStarting(with: keyFragment, tableName: TABLE_FLOAT, tag: tag) { statement in
            Float(sqlite3_column_double(statement, 1))
        }
    }

    public func getAllIntValuesStarting(with keyFragment:String, tag: String) -> Result<[String: Int], SQLiteError> {
        return getAllValuesStarting(with: keyFragment, tableName: TABLE_INT, tag: tag) { statement in
            Int(sqlite3_column_int(statement, 1))
        }
    }

    public func getAllBooleanValuesStarting(with keyFragment:String, tag: String) -> Result<[String: Bool], SQLiteError> {
        return getAllValuesStarting(with: keyFragment, tableName: TABLE_BOOLEAN, tag: tag) { statement in
            sqlite3_column_int(statement, 1) != 0
        }
    }

    public func getAllLongValuesStarting(with keyFragment:String, tag: String) -> Result<[String: Int64], SQLiteError> {
        return getAllValuesStarting(with: keyFragment, tableName: TABLE_LONG, tag: tag) { statement in
